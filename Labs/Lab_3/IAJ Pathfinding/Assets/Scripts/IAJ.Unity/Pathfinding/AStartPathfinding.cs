...
namespace Assets.Scripts.IAJ.Unity.Pathfinding
{
    public class AStarPathfinding
    {
        ...
        protected virtual void ProcessChildNode(NodeRecord bestNode, NavigationGraphEdge connectionEdge)
        {
            ...
            var openNode = this.Open.Search(childNode);
            var closedNode = this.Closed.Search(childNode);
        ...
        public bool Search(out Path solution, bool returnPartialSolution)
        {
            int nodesProcessed = 0;

            while (nodesProcessed < this.NodesPerSearch)
            {
                if (this.Open.Count() > 0)
                {
                    ...
                    this.MaxOpenNodes = Mathf.Max(this.Open.Count(), this.MaxOpenNodes);
                }
                else
                {
                    this.InProgress = false;
